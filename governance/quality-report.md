...
- `decision_plane_component`: 335
- `revocation_supported`: 113
- `with_evidence`: 563
- `with_see_also`: 563
- `with_sources`: 563
...
- `missing_sources`: 0 — No source references are present.
- `missing_see_also`: 0 — No cross-reference terms are present.
- `missing_evidence`: 0 — No evidence artifacts are present.
- `generic_evidence`: 0 — Only generic definition-change evidence is present for a non-descriptive term.
- `weak_revocation_evidence`: 0 — Revocation semantics are not backed by revocation-relevant evidence.
- `decision_plane_low_assurance`: 0 — Decision-plane role is paired with a low assurance hint.
- `high_impact_informative`: 0 — High-impact decision point is paired with informative assurance.
